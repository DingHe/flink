 ...
 */
// WatermarkGenerator 是决定“时间进度”的核心逻辑组件。
// 如果说 TimestampAssigner 是从数据中提取时间，那么 WatermarkGenerator 就是根据提取到的时间计算并发出水位线（Watermark），从而告诉 Flink：“此时此刻，早于这个时间点的数据应该都已经到齐了。”
...
public interface WatermarkGenerator<T> {
     ...
     */
    // 触发时机：由 Flink 框架周期性地调用。
    // 调用频率由 execution.checkpointing.adaptive-timeout 或 setAutoWatermarkInterval（默认 200ms）决定。
